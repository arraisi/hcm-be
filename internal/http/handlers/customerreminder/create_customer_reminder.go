...
func (h *Handler) CreateCustomerReminder(w http.ResponseWriter, r *http.Request) {
	...
	// Parse JSON body
	var requestBody customerreminder.Request
	...
	if err := validator.ValidateStruct(requestBody); err != nil {
	...
	if err := h.idempotencySvc.Store(requestBody.EventID); err != nil {
		errorResponse := errors.NewErrorResponseFromList(errors.ErrWebhookIdempotencyFailed, errors.ErrListWebhook)
		response.ErrorResponseJSON(w, errorResponse)
		return
	}

	err = h.svc.CreateCustomerReminder(r.Context(), requestBody)
	if err != nil {
