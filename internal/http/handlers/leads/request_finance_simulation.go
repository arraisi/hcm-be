...
// RequestFinanceSimulation handles POST /webhook/finance-simulation
func (h *Handler) RequestFinanceSimulation(w http.ResponseWriter, r *http.Request) {
	...
	// Parse JSON body
	var financeSimEvent leads.FinanceSimulationWebhookEvent
	if err := json.Unmarshal(body, &financeSimEvent); err != nil {
	...
	//if err := validator.ValidateStruct(financeSimEvent); err != nil {
	...
	if err := h.idempotencySvc.Store(financeSimEvent.EventID); err != nil {
		errorResponse := errors.NewErrorResponseFromList(errors.ErrWebhookIdempotencyFailed, errors.ErrListWebhook)
		response.ErrorResponseJSON(w, errorResponse)
		return
	}

	err = h.svc.RequestFinanceSimulation(r.Context(), financeSimEvent)
	if err != nil {
	...
	httpResp := webhookDto.Response{
		Data: webhookDto.ResponseData{
			EventID: financeSimEvent.EventID,
			Status:  "RECEIVED",
		},
		Message: "accepted",
	}
